...
# ── Coin whitelist — sadece likit büyük coinler ───────────────────────────────
...
# ── State ─────────────────────────────────────────────────────────────────────
_prices     = {}
_positions  = []   # OKX'ten gelen gerçek pozisyonlar
_stats      = {}
_last_fetch = 0
...
def _okx_sign(ts, method, path, body=""):
    ...
    return {"OK-ACCESS-KEY": OKX_KEY, "OK-ACCESS-SIGN": sig,
            ...
            "Content-Type": "application/json", "User-Agent": "CryptoBot/2.0"}
    ...
    with _lock:
        ...
        if OKX_KEY:
            ...
            for p in data.get("data", []):
                ...
                # Manuel pozisyonlara dokunma — is_adopted=True ile işaretle
                ...
                # Bot açtı mı manuel mi? — OKX'te bunu ayırt etmek için
                # is_adopted = True → manuel, bot müdahale etmez
                ...
                pos_list.append({
                    ...
                    "stop_loss":   round(entry*(0.97 if side=="long" else 1.03),4),
                    "take_profit": round(entry*(1.04 if side=="long" else 0.96),4),
                    "notional": round(notional,2),
                    "candles_held": 1, "max_bars": 10, "bars_remaining": 9,
                    "lot_type": "standard",
                    "is_adopted": is_manual,  # Manuel pozisyon işareti
                    "distance_to_sl_pct":  round(abs(entry*(0.97 if side=="long" else 1.03)-current)/current*100,2),
                    "distance_to_tp_pct":  round(abs(entry*(1.04 if side=="long" else 0.96)-current)/current*100,2),
                    "distance_to_be_pct":  round(pnl_p,2),
                    "distance_to_tp1_pct": round(abs(entry*(1.015 if side=="long" else 0.985)-current)/current*100,2),
                    "strategy_name": "Manuel" if is_manual else "Smart Scalping v2",
...
@app.get("/api/health")
def health():
    return {"ok": True, "exchange": "okx", "authenticated": bool(OKX_KEY),
