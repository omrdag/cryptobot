...
def bot_loop():
    ...
    while True:
        try:
            ...
            try:
                # Tüm açık pozisyonlar (restart recovery dahil)
                _all_open_insts = {p.get("instId") for p in positions}
                _open_longs     = {p.get("instId") for p in positions if p.get("side")=="long"}
                _open_cnt       = len(positions)
                _long_cnt       = len(_open_longs)

                _log(f"[EMIR] signals:{len(signals)} pos:{_open_cnt}/{MAX_POSITIONS} long:{_long_cnt}/{MAX_POSITIONS} rejim:{current_regime}")

                for _esym, _esig in signals.items():
                    if _open_cnt >= MAX_POSITIONS:
                        _log(f"⛔ Max pozisyon dolu ({_open_cnt}/{MAX_POSITIONS})")
                        break

                    _einst  = _esig.get("inst_id","")
                    _elong  = _esig.get("long",{})
                    _lenter = bool(_elong.get("enter", False))
                    _lscore = int(_elong.get("score", 0) or 0)
                    ...
                    _log(f"[EMIR-CHK] {_esym} enter={_lenter} score={_lscore} entry={_lentry:.4f} min={LONG_MIN_SCORE} rsi={_lrsi:.1f} maxrsi={LONG_RSI_MAX}")

                    if _einst in _all_open_insts:
                        _log(f"⏭ {_esym} zaten açık — atla")
                        continue
                    if not _lenter:
                        _log(f"⏭ {_esym} enter=False — atla")
                        continue
                    if _lentry <= 0:
                        _log(f"⏭ {_esym} entry=0 — atla")
                        continue
                    if _lscore < LONG_MIN_SCORE:
                        _log(f"⏭ {_esym} skor {_lscore}<{LONG_MIN_SCORE} — atla")
                        continue
                    if _long_cnt >= MAX_POSITIONS:
                        _log(f"⛔ Long limit dolu ({_long_cnt}/{MAX_POSITIONS})")
                        break
                    if current_regime == "NO_TRADE":
                        _log(f"⛔ NO_TRADE rejimi — emir açılmıyor")
                        break
                    if _lrsi >= LONG_RSI_MAX:
                        _log(f"⏭ {_esym} RSI {_lrsi:.1f}>={LONG_RSI_MAX} — atla")
                        continue
                    ...
                    _log(f"🟢 {_esym} LONG | Puan:{_lscore}/11 | RSI:{_lrsi:.1f} | Giriş:${_lentry:.4f} SL:${_sl:.4f} TP:${_tp:.4f} | {LEVERAGE}x")
                    ...
                    if _ok:
                        ...
                        _all_open_insts.add(_einst)
                        _open_longs.add(_einst)
                        _open_cnt += 1
                        _long_cnt += 1
                        with _lock:
                ...
                import traceback
                _log(f"[EMIR] Hata: {_ee} | {traceback.format_exc()[:400]}", "warning")
